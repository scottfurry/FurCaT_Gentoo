...
# shamelessly borrowed from Gentoo ebuild
# Usage: arch_src_uri <gentoo arch> <upstream arch>
arch_src_uri() {
	echo "${1}? (
		https://github.com/VSCodium/${PN_SHRT}/releases/download/${PV}/VSCodium-linux-${2}-${PV}.tar.gz -> ${P}-${1}.tar.gz
	)"
}
...
src_install() {
	...
	# Install
	pax-mark m codium
	mkdir -p "${ED}/opt/${PN_SHRT}" || die
	cp -r . "${ED}/opt/${PN_SHRT}" || die
	fperms 4711 /opt/${PN_SHRT}/chrome-sandbox

	dosym -r "/opt/${PN_SHRT}/bin/codium" "usr/bin/vscodium"
	dosym -r "/opt/${PN_SHRT}/bin/codium" "usr/bin/codium"
	...
	sed "s|@exec_extra_flags@|${EXEC_EXTRA_FLAGS[*]}|g" \
		"${FILESDIR}/${PN_SHRT}-url-handler.desktop" \
		> "${T}/${PN_SHRT}-url-handler.desktop" || die

	sed "s|@exec_extra_flags@|${EXEC_EXTRA_FLAGS[*]}|g" \
		"${FILESDIR}/${PN_SHRT}.desktop" \
		> "${T}/${PN_SHRT}.desktop" || die

	sed "s|@exec_extra_flags@|${EXEC_EXTRA_FLAGS[*]}|g" \
		"${FILESDIR}/${PN_SHRT}-open-in-new-window.desktop" \
		> "${T}/${PN_SHRT}-open-in-new-window.desktop" || die

	domenu "${T}/${PN_SHRT}.desktop"
	domenu "${T}/${PN_SHRT}-url-handler.desktop"
	domenu "${T}/${PN_SHRT}-open-in-new-window.desktop"
